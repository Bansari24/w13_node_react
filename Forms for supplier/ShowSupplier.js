...
//----------------------------------------------Show all suppliers------------------------------------------------------------------- 
showSuppliers = () => {
        fetch('http://localhost:8005/supplier/allSupplier').then(
            response => {
                if (response.ok) {     
...
searchSupplier = (supplier_Name) => {
    fetch('http://localhost:8005/supplier/searchSupplierName/'+supplier_Name,{
        method: "GET"
    }).then(
        response => {
            if (response.ok) {     
...
//--------------TO DELETE A PRODUCT------------------------------------------
delete=(id)=>{
    fetch("http://localhost:8005/supplier/delete/" + id, {
        method: "DELETE"
    });
}
//------------------------------------------------------------CALL SHOW SUPPLIER---------------------------------------------------------------------
...
{/*--------------------------search supplier--------------------------   */}
                ...
                <form className="example" action="/action_page.php" style={{ margin: "auto", maxWidth: "400px" }} >
                <input type="text" id="searchname" placeholder="Search Supplier" name="search" />
                    <button type="button" onClick={() => this.searchSupplier(document.getElementById("searchname").value)} style={{marginTop:"-40px",marginLeft:"250px"}}>
